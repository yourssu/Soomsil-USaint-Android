...
import com.yourssu.soomsil.usaint.data.source.local.entity.*
...
class ReportCardRepository(
    ...
    // TotalReportCard
    suspend fun upsertTotalReportCard(totalReportCard: TotalReportCard) = withContext(Dispatchers.IO) {
        totalReportCardDao.insertTotalReportCard(totalReportCard)
    }

    suspend fun getTotalReportCard(): TotalReportCard? = withContext(Dispatchers.IO) {
        totalReportCardDao.getTotalReportCard()
    }

    suspend fun getTotalReportCardWithSemesters(): TotalReportCardWithSemesters? = withContext(Dispatchers.IO) {
        totalReportCardDao.getTotalReportCardWithSemesters()
    }


    // Semester
    suspend fun upsertSemester(semester: Semester) = withContext(Dispatchers.IO) {
        semesterDao.insertSemester(semester)
    }

    suspend fun getSemester(year: Int, semesterName: String): Semester? = withContext(Dispatchers.IO) {
        semesterDao.getSemesterByYearAndSemester(year, semesterName)
    }

    suspend fun getSemestersByTotalReportCardId(totalReportCardId: Int): List<Semester> = withContext(Dispatchers.IO) {
        semesterDao.getSemestersByTotalReportCardId(totalReportCardId)
    }

    suspend fun getSemesterWithLectures(year: Int, semesterName: String): SemesterWithLectures? = withContext(Dispatchers.IO) {
        semesterDao.getSemesterWithLectures(year, semesterName)
    }


    // Lecture 관련 로직
    suspend fun upsertLecture(lecture: Lecture) = withContext(Dispatchers.IO) {
        lectureDao.insertLecture(lecture)
    }

    suspend fun getLectureByCode(code: String): Lecture? = withContext(Dispatchers.IO) {
        lectureDao.getLectureByCode(code)
    }

    suspend fun getLecturesBySemesterId(semesterId: Int): List<Lecture> = withContext(Dispatchers.IO) {
